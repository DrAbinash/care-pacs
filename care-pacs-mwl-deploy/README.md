...
|---|---|---|
| `care_erp_sync.py` | `ERP_BASE_URL=http://172.16.1.139:8888` (stale) | → `http://192.168.1.137:8888` |
...
| `auto_pull.lua` | Empty stub (only logs store) | Leave as-is — `care_erp_sync` owns ERP push |
...
4. After scan: `care-erp-sync` logs `ERP study POST … status=200`; worklist match is GREEN on accession even if name order differs.
